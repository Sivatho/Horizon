...
namespace ClientServicing.Main.AbstractComponents.API.ValidationMethods.AccountHistory.PolicyAccountingHistory
{
    public class PolicyAccountingHistoryValidationMethods : AbstractValidationMethods, IPolicyAccountingHistoryValidationMethods
    {
        ...
        public void ValidatePolicyAccountingHistorResponseDataIsNotNullOrEmpty(PolicyAccountHistoryResponse policyAccountHistoryResponse)
        {
            ...
            {
                //Response Content
                Assert.That(policyAccountHistoryResponse.executionOutcome, Is.Not.Null.Or.Empty, "Fetch Banks Response: Response Message Should Not Be Null Or Empty");
                Assert.That(policyAccountHistoryResponse.data, Is.Not.Null, "Fetch Banks Response: Data should not be null");
                                    ...
                                    foreach (var policy in item.Value.EnumerateArray()) {
                                        ...
                                        foreach (var p in policy.EnumerateObject()) {
                                            switch (p.Name) {
                                                ...
                                                case "bankSubmissionDate":  policyResult.bankSubmissionDate =       (DateTime?)utilitiesHelper.ReadDateTimeNullable(p.Value); break;
                                                case "strikeDate":          policyResult.strikeDate =               (DateTime?)utilitiesHelper.ReadDateTimeNullable(p.Value); break;
                                                case "paymentDate":         policyResult.paymentDate =              (DateTime)utilitiesHelper.ReadDateTimeNullable(p.Value); break;
                                                case "trackingDays":        policyResult.trackingDays =             (int?)utilitiesHelper.ReadInt32Nullable(p.Value); break;
