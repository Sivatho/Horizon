...
namespace ClientServicing.Main.AbstractComponents.API.ValidationMethods.PolicyDocument
{
    public class CheckPolicyDocumentExistValidationMethods : AbstractValidationMethods, ICheckPolicyDocumentExistValidationMethods
    {
        ...
        public override void ValidateResponseFieldParametersIsValid(RestResponse restResponse)
        {
            ...
            Assert.That(root.ValueKind, Is.EqualTo(JsonValueKind.Object), "Response Content must be a JSON object.");

            var expectedKeys = new[] { "succeeded", "message", "errors", "data" };

            // Collect actual keys
            var actualKeys = root.EnumerateObject().Select(p => p.Name).ToArray();

            // Assert exact keys (no extras, no missing, order-insensitive)
            Assert.That(actualKeys, Is.EquivalentTo(expectedKeys),
                $"Response must contain exactly these keys: {string.Join(", ", expectedKeys)}");
